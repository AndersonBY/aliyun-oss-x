...
Server: AliyunOSS"""
    ...
    def test_get_with_progress(self, do_request):
        ...
        self.assertEqual(content_read, aliyun_oss_x.to_bytes(content))
...
{1}""".format(len(content), content.decode() if isinstance(content, bytes) else content)
    ...
    def test_append_with_progress(self, do_request):
        ...
        result = bucket().append_object(
            "sjbhlsgsbecvlpbf", 0, content, progress_callback=self.progress_callback, init_crc=0
        )
...
x-oss-request-id: 566B6C0D8CDE4E975D730BEF"""
    ...
    def test_batch_delete_empty(self):
        self.assertRaises(aliyun_oss_x.exceptions.ClientError, bucket().batch_delete_objects, [])
