...
def bucket(crypto_provider=None):
    if crypto_provider:
        return aliyun_oss_x.CryptoBucket(
...
def service():
    return aliyun_oss_x.Service(
        aliyun_oss_x.Auth("fake-access-key-id", "fake-access-key-secret"), "http://oss-cn-hangzhou.aliyuncs.com"
    )
...
def do4body(req, timeout, req_info=None, data_type=DT_BYTES, status=200, body=None, content_type=None):
    if content_type:
...
def make_do4body(req_infos=None, body_list=None):
    ...
    def do4body_func(req, timeout):
        result = do4body(req, timeout, req_info=req_infos[i.var], body=body_list[i.var])
        i.var += 1
        return result
...
def do4response(req, timeout, req_info=None, payload=None):
    if req_info:
        ...
        if req.data is None:
            ...
            req_info.data = aliyun_oss_x.to_bytes(req.data)
...
def do4put_object(req, timeout, req_info=None, data_type=DT_BYTES):
    return do4put(
        req, timeout, in_headers={"ETag": '"E5831D5EBC7AAF5D6C0D20259FE141D2"'}, req_info=req_info, data_type=data_type
    )
...
class MockRequest:
    def __init__(self, request_text):
        ...
        if len(fields) == 2:
            self.body = aliyun_oss_x.to_bytes(fields[1])
        else:
            self.body = b""
...
class OssTestCase(unittest.TestCase):
    def __init__(self, *args, **kwargs):
        ...
        self.default_request_retries = aliyun_oss_x.defaults.request_retries
        self.default_multipart_threshold = aliyun_oss_x.defaults.multipart_threshold
        self.default_part_size = aliyun_oss_x.defaults.part_size

    def setUp(self):
        aliyun_oss_x.defaults.connect_timeout = self.default_connect_timeout
        aliyun_oss_x.defaults.request_retries = self.default_request_retries
        aliyun_oss_x.defaults.multipart_threshold = self.default_multipart_threshold
        aliyun_oss_x.defaults.part_size = self.default_part_size
    ...
    def assertXmlEqual(self, a, b):
        ...
        normalized_a = minidom.parseString(aliyun_oss_x.to_bytes(a)).toxml(encoding="utf-8")
        normalized_b = minidom.parseString(aliyun_oss_x.to_bytes(b)).toxml(encoding="utf-8")
