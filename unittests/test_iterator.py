...
class TestIterator(OssTestCase):
    ...
    def test_bucket_iterator_not_truncated(self, do_request):
        ...
        expected = [
            ...
            SimplifiedBucketInfo(
                ...
                "oss-us-west-internal-1.aliyuncs.com",
                "Standard",
            ),
        ]

        got = list(aliyun_oss_x.BucketIterator(service()))
    ...
    def test_bucket_iterator_truncated(self, do_request):
        ...
        got = list(aliyun_oss_x.BucketIterator(service(), max_keys=2))
    ...
    def test_object_iterator_empty(self, do_request):
        ...
        got = list(aliyun_oss_x.ObjectIterator(bucket(), max_keys=1000))
    ...
    def test_object_iterator_not_truncated(self, do_request):
        ...
        expected = [
            SimplifiedObjectInfo(
                ...
                4308,
                "Standard",
                Owner("1047205513514293", "1047205513514293"),
            ),
            SimplifiedObjectInfo(
                "object-2",
                1435053415,
            ...
            SimplifiedObjectInfo(
                "object-3",
                1421412094,
                "B28F7255E6EA777DB0AFB1C58C2CFCFE",
                "Normal",
                10718416,
                "Standard",
                Owner("1047205513514293", "1047205513514293"),
    ...
    def test_object_iterator_truncated(self, do_request):
        ...
        got = list(aliyun_oss_x.ObjectIterator(bucket(), max_keys=1))
    ...
    def test_upload_iterator_empty(self, do_request):
        ...
        got = list(aliyun_oss_x.MultipartUploadIterator(bucket(), max_uploads=1000))
        self.assertEqual(len(got), 0)

    def test_part_iterator_default_max_retries(self):
        iter = aliyun_oss_x.PartIterator(bucket(), "fake-key", "fake-upload-id")
        self.assertEqual(iter.max_retries, aliyun_oss_x.defaults.request_retries)

        aliyun_oss_x.defaults.request_retries = 100
        iter = aliyun_oss_x.PartIterator(bucket(), "fake-key", "fake-upload-id")
        self.assertEqual(iter.max_retries, 100)

        iter = aliyun_oss_x.PartIterator(bucket(), "fake-key", "fake-upload-id", max_retries=1)
        self.assertEqual(iter.max_retries, 1)

    def test_object_iterator_default_max_retries(self):
        iter = aliyun_oss_x.ObjectIterator(bucket())
        self.assertEqual(iter.max_retries, aliyun_oss_x.defaults.request_retries)
        ...
        iter = aliyun_oss_x.ObjectIterator(bucket(), max_retries=1)
        self.assertEqual(iter.max_retries, 1)

    def test_bucket_iterator_default_max_retries(self):
        iter = aliyun_oss_x.BucketIterator(service())
        self.assertEqual(iter.max_retries, aliyun_oss_x.defaults.request_retries)
        ...
        iter = aliyun_oss_x.BucketIterator(service(), max_retries=1)
        self.assertEqual(iter.max_retries, 1)

    def test_object_upload_iterator_default_max_retries(self):
        iter = aliyun_oss_x.ObjectUploadIterator(bucket(), "fake-key")
        self.assertEqual(iter.max_retries, aliyun_oss_x.defaults.request_retries)
        ...
        iter = aliyun_oss_x.ObjectUploadIterator(bucket(), "fake-key", max_retries=1)
        self.assertEqual(iter.max_retries, 1)
