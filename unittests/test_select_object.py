...
from aliyun_oss_x.exceptions import SelectOperationClientError
from aliyun_oss_x.exceptions import InconsistentError
from aliyun_oss_x.exceptions import SelectOperationFailed
from aliyun_oss_x.exceptions import ClientError

from unittests.common import *


class ContiniousFrame:
...
class EndFrame(ContiniousFrame):
    def __init__(self, offset, scanned_size, status, error=b""):
        super(EndFrame, self).__init__(
            offset, struct.pack("!QI", scanned_size, status) + error, len(error) + 20, ContiniousFrame._END_FRAME_TYPE
        )
...
class EndMetaFrame(ContiniousFrame):
    def __init__(self, offset, scannedsize, status, splits, rows, cols, error):
        super(EndMetaFrame, self).__init__(
            offset,
            struct.pack("!QIIQI", scannedsize, status, splits, rows, cols) + error,
            36 + len(error),
            ContiniousFrame._META_END_FRAME_TYPE,
        )
...
class EndJsonMetaFrame(ContiniousFrame):
    def __init__(self, offset, scannedsize, status, splits, rows, error):
        super(EndJsonMetaFrame, self).__init__(
            offset,
            struct.pack("!QIIQ", scannedsize, status, splits, rows) + error,
            32 + len(error),
            ContiniousFrame._JSON_META_END_FRAME_TYPE,
        )
        self.scanned_size = scannedsize
        self.status = status
        self.splits = splits
...
def generate_data(resp_content, output_raw, error=b"", status=206, simulate_bad_crc=False):
    if output_raw:
        ...
        if simulate_bad_crc:
            frameStr = (
                frameStr[0 : len(frameStr) - 2] + b"X"
            )  # 'X' is the character that does not exist in the content
