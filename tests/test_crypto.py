...
import aliyun_oss_x
import unittests
from aliyun_oss_x import LocalRsaProvider, AliKMSProvider, RsaProvider, compat
from aliyun_oss_x.utils import silently_remove
from aliyun_oss_x.exceptions import OpenApiServerError, OpenApiFormatError, ClientError
...
class TestCrypto(unittests.common.OssTestCase):
    ...
    def test_rsa_provider_init_invalid_keys(self):
        ...
        with open("./rsa-test.private_key.pem", "wb") as f:
            f.write(aliyun_oss_x.to_bytes(private_key))
    ...
    def test_local_rsa_provider_diff_keys(self):
        ...
        with patch.object(aliyun_oss_x.utils, "random_key", return_value=plain_key):
            with patch.object(aliyun_oss_x.utils, "random_iv", return_value=plain_iv):
                ...
                self.assertFalse(content_crypto_material.is_unencrypted())
                self.assertRaises(
                    ClientError, provider_diff.decrypt_encrypted_key, content_crypto_material.encrypted_key
                )
                self.assertRaises(
                    ClientError, provider_diff.decrypt_encrypted_iv, content_crypto_material.encrypted_iv
                )
    ...
    def test_ali_kms_provider_basic(self):
        ...
        with patch("aliyun_oss_x.AliKMSProvider.get_key", return_value=[plain_key, encrypted_key]):
            with patch.object(aliyun_oss_x.utils, "random_iv", return_value=plain_iv):
                content_crypto_material = provider.create_content_material()
                self.assertFalse(content_crypto_material.is_unencrypted())
                decrypted_key = provider.decrypt_encrypted_key(content_crypto_material.encrypted_key)
    ...
    def get_cmk_id(self):
        ...
        try:
            ...
            bucket = aliyun_oss_x.Bucket(aliyun_oss_x.make_auth(OSS_ID, OSS_SECRET), OSS_ENDPOINT, bucketName)
