...
class TestWebsite(OssTestCase):
    def test_normal_set_website_without_routing_rule(self):
        ...
        self.assertRaises(aliyun_oss_x.exceptions.NoSuchWebsite, self.bucket.get_bucket_website)
    ...
    def test_normal_set_website_with_mirror(self):
        ...
        condition1 = Condition(
            key_prefix_equals="1~!@#$%^&*()-_=+|\\[]{}<>,./?`~",
            http_err_code_return_equals=404,
            include_header_list=[include_header1, include_header2],
        )
        condition2 = Condition(
            key_prefix_equals="2~!@#$%^&*()-_=+|\\[]{}<>,./?`~",
            http_err_code_return_equals=404,
            include_header_list=[include_header1, include_header2],
        )
        ...
        mirror_header = RedirectMirrorHeaders(
            pass_all=True, pass_list=pass_list, remove_list=remove_list, set_list=set_list
        )

        # direct type Mirror
        redirect1 = Redirect(
            redirect_type="Mirror",
            mirror_url="http://www.test.com/",
        ...
        redirect2 = Redirect(
            ...
            mirror_url="http://www.test.com/",
            mirror_url_slave="http://www.slave.com/",
            mirror_url_probe="http://www.test.com/index.html",
    ...
    def test_normal_set_website_with_alicdn_external_internal(self):
        ...
        redirect1 = Redirect(
            redirect_type=REDIRECT_TYPE_ALICDN,
            pass_query_string=True,
            replace_key_with="${key}.suffix",
            proto="http",
            host_name="oss.aliyuncs.com",
            http_redirect_code=302,
        )

        # test direct type  External
        redirect2 = Redirect(
            redirect_type=REDIRECT_TYPE_EXTERNAL,
            pass_query_string=False,
        ...
        redirect3 = Redirect(
            redirect_type=REDIRECT_TYPE_INTERNAL, pass_query_string=False, replace_key_with="${key}.suffix"
        )
    ...
    def test_unnormal_routing_rule(self):
        ...
        self.assertRaises(
            aliyun_oss_x.exceptions.ClientError, RoutingRule, rule_num="str", condition=condition, redirect=redirect
        )

        # rule_num < 0
        self.assertRaises(
            aliyun_oss_x.exceptions.ClientError, RoutingRule, rule_num=-1, condition=condition, redirect=redirect
        )

        # rule_num is none
        self.assertRaises(
            aliyun_oss_x.exceptions.ClientError, RoutingRule, rule_num=None, condition=condition, redirect=redirect
        )

        # condition is none
        self.assertRaises(
            aliyun_oss_x.exceptions.ClientError, RoutingRule, rule_num=1, condition=None, redirect=redirect
        )

        # redirect is none
        self.assertRaises(
            aliyun_oss_x.exceptions.ClientError, RoutingRule, rule_num=1, condition=condition, redirect=None
        )

        # condition.http_err_code_return_equals is not 404 when reidrect type is mirror
        t_condition = Condition(
            key_prefix_equals="~!@#$%^&*()-_=+|\\[]{}<>,./?`~",
            http_err_code_return_equals=400,
            include_header_list=[include_header1, include_header2],
        )
        self.assertRaises(
            aliyun_oss_x.exceptions.ClientError, RoutingRule, rule_num=1, condition=t_condition, redirect=redirect
        )

        # rules type is not list
        self.assertRaises(aliyun_oss_x.exceptions.ClientError, BucketWebsite, index_file, error_file, rule)
    ...
    def test_unnormal_rule_conditon(self):
        ...
        self.assertRaises(aliyun_oss_x.exceptions.ClientError, Condition, "test", 404, "str")
    ...
    def test_unnormal_redirect_mirror(self):
        ...
        redirect1 = Redirect(
            redirect_type=REDIRECT_TYPE_MIRROR,
            mirror_url="http://www.test.com/",
            mirror_url_slave="https://www.slave.com/",
            mirror_url_probe="http://www.test.com/index.html",
            mirror_pass_query_string=True,
            mirror_follow_redirect=True,
            mirror_check_md5=False,
        ...
        self.assertRaises(
            aliyun_oss_x.exceptions.ClientError,
            Redirect,
            redirect_type="unnormal-direct-type",
            mirror_url="http://www.test.com/",
            mirror_pass_query_string=True,
            mirror_follow_redirect=True,
            mirror_check_md5=False,
        ...
        self.assertRaises(
            ...
            redirect_type=REDIRECT_TYPE_MIRROR,
            mirror_url="http://www.test.com/",
            mirror_url_slave="www.slave.com/",
            mirror_url_probe="http://www.test.com/index.html",
            mirror_pass_query_string=True,
            mirror_follow_redirect=True,
            mirror_check_md5=False,
        ...
        self.assertRaises(aliyun_oss_x.exceptions.ClientError, RedirectMirrorHeaders, pass_list="a")
        # pass_list capacity > 10
        pass_list = ["key1", "key2", "key3", "key4", "key5", "key6", "key7", "key8", "key9", "key10", "key11"]
        self.assertRaises(aliyun_oss_x.exceptions.ClientError, RedirectMirrorHeaders, pass_list=pass_list)

        # correct remove_list
        mirror_header = RedirectMirrorHeaders(remove_list=["a", "b"])
        # remove_list type is not list
        self.assertRaises(aliyun_oss_x.exceptions.ClientError, RedirectMirrorHeaders, remove_list="str")
        # remove_list capacity > 10
        remove_list = ["key1", "key2", "key3", "key4", "key5", "key6", "key7", "key8", "key9", "key10", "key11"]
        self.assertRaises(aliyun_oss_x.exceptions.ClientError, RedirectMirrorHeaders, remove_list=remove_list)
        ...
        self.assertRaises(aliyun_oss_x.exceptions.ClientError, RedirectMirrorHeaders, set_list=t_set)
        # mirror_header set_list capacity > 10
        set_list = [t_set, t_set, t_set, t_set, t_set, t_set, t_set, t_set, t_set, t_set, t_set]
        self.assertRaises(aliyun_oss_x.exceptions.ClientError, RedirectMirrorHeaders, set_list=set_list)
    ...
    def test_unnormal_redirect_alicdn_external_internal(self):
        ...
        self.assertRaises(
            aliyun_oss_x.exceptions.ClientError,
            Redirect,
            redirect_type=REDIRECT_TYPE_INTERNAL,
            pass_query_string=False,
            replace_key_with="${key}.suffix",
            host_name="oss.aliyuncs.com",
        )
