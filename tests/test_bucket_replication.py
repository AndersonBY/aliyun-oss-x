...
import aliyun_oss_x
from aliyun_oss_x.models import ReplicationRule
...
class TestBucketReplication(OssTestCase):
    def setUp(self):
        ...
        self.source_bucket = aliyun_oss_x.Bucket(
            aliyun_oss_x.make_auth(OSS_ID, OSS_SECRET), OSS_ENDPOINT, source_bucket_name, region=OSS_REGION
        )
        ...
        self.replica_bucket = aliyun_oss_x.Bucket(
            aliyun_oss_x.make_auth(OSS_ID, OSS_SECRET),
            self.replica_endpoint + ".aliyuncs.com",
            self.replica_bucket_name,
            region="ap-south-1",
        )
        ...
        self.sync_role_name = (
            OSS_INVENTORY_BUCKET_DESTINATION_ARN[index + 1 :] if index != -1 else OSS_INVENTORY_BUCKET_DESTINATION_ARN
        )
    ...
    def test_put_multi_replica_rule(self):
        ...
        self.assertRaises(
            aliyun_oss_x.exceptions.BucketReplicationAlreadyExist,
            self.source_bucket.put_bucket_replication,
            replica_config,
        )
    ...
    def test_replica_rule_id(self):
        self.assertRaises(
            aliyun_oss_x.exceptions.NoSuchReplicationRule, self.source_bucket.get_bucket_replication_progress, "wrong"
        )
