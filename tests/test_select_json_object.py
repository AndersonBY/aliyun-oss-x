...
class SelectJsonObjectTestHelper:
...
class TestSelectJsonObject(OssTestCase):
    def test_select_json_document_democrat_senators(self):
        ...
        content = helper.test_select_json_object(
            self, "select * from ossobject.objects[*] where party = 'Democrat'", input_format
        )
    ...
    def test_select_json_lines_democrat_senators(self):
        ...
        content = helper.test_select_json_object(
            self, "select * from ossobject where party = 'Democrat'", input_format
        )
    ...
    def test_select_gzip_json_object_like(self):
        ...
        content = helper.test_select_json_object(
            self,
            "select person.firstname, person.lastname from ossobject.objects[*]  where person.birthday like '1959%'",
            select_params,
        )
        content = content[0 : len(content) - 1]  # remove the last ','
        content = b"[" + content + b"]"  # make json parser happy
    ...
    def test_select_json_object_line_range(self):
        ...
        content = helper.test_select_json_object(
            self, "select person.firstname as aaa as firstname, person.lastname, extra from ossobject'", select_params
        )
        content = content[0 : len(content) - 1]  # remove the last ','
        content = b"[" + content + b"]"  # make json parser happy
        result = json.loads(content.decode("utf-8"))
    ...
    def test_select_json_object_int_aggregation(self):
        ...
        content = helper.test_select_json_object(
            self,
            "select avg(cast(person.cspanid as int)), max(cast(person.cspanid as int)), min(cast(person.cspanid as int)) from ossobject.objects[*] where person.cspanid = 1011723",
            select_params,
        )
    ...
    def test_select_json_object_float_aggregation(self):
        ...
        content = helper.test_select_json_object(
            self,
            "select avg(cast(person.cspanid as double)), max(cast(person.cspanid as double)), min(cast(person.cspanid as double)) from ossobject.objects[*]",
            select_params,
        )
        print(content)
    ...
    def test_select_json_object_concat(self):
        ...
        content = helper.test_select_json_object(
            self,
            "select person from ossobject.objects[*] where (person.firstname || person.lastname) = 'JohnKennedy'",
            select_params,
        )
        content = content[0 : len(content) - 1]  # remove the last ','
        content = b"[" + content + b"]"  # make json parser happy
    ...
    def test_select_json_object_complicate_condition(self):
        ...
        content = helper.test_select_json_object(
            self,
            "select person.firstname, person.lastname, congress_numbers from ossobject where startdate > '2017-01-01' and senator_rank = 'junior' or state = 'CA' and party = 'Republican' ",
            select_params,
        )
        content = content[0 : len(content) - 1]  # remove the last ','
        content = b"[" + content + b"]"  # make json parser happy
    ...
    def test_select_json_object_invalid_sql(self):
        ...
        select_params = {"Json_Type": "LINES"}
        helper.test_select_json_object_invalid_request(
            self, "select * from ossobject where avg(cast(person.birthday as int)) > 2016", select_params
        )
        helper.test_select_json_object_invalid_request(self, "", select_params)
        helper.test_select_json_object_invalid_request(
            self, "select person.lastname || person.firstname from ossobject", select_params
        )
        helper.test_select_json_object_invalid_request(
            self, "select * from ossobject group by person.firstname", select_params
        )
        helper.test_select_json_object_invalid_request(self, "select * from ossobject order by _1", select_params)
        helper.test_select_json_object_invalid_request(
            self, "select * from ossobject oss join s3object s3 on oss.CityName = s3.CityName", select_params
        )
    ...
    def test_select_json_object_parse_num_as_string(self):
        ...
        result = self.bucket.select_object(
            key,
            "select a from ossobject where cast(a as decimal) = 123456789.1234567890",
            None,
