...
Location: /ming-oss-share"""
        ...
        unittests.common.bucket().create_bucket(aliyun_oss_x.BUCKET_ACL_PRIVATE)
...
x-oss-request-id: 566B6BD9B295345D15740F1F"""
        ...
        rule = LifecycleRule(
            ...
            storage_transitions=[
                StorageTransition(created_before_date=date1, storage_class=aliyun_oss_x.BUCKET_STORAGE_CLASS_STANDARD)
            ],
...
x-oss-request-id: 566B6BDB1BA604C27DD419B8"""
        ...
        rule = LifecycleRule(
            ...
            storage_transitions=[
                StorageTransition(days=days, storage_class=aliyun_oss_x.BUCKET_STORAGE_CLASS_STANDARD)
            ],
    ...
    <Expiration>
      <Date>{3}</Date>
        ...
        req_info = unittests.common.mock_response(
            do_request, response_text.format(id, prefix, status, "2015-12-25T00:00:00.000Z")
        )
...
<LifecycleConfiguration>
  <Rule>
    ...
    <Transition>
        ...
        self.assertEqual(rule.storage_transitions[0].storage_class, aliyun_oss_x.BUCKET_STORAGE_CLASS_STANDARD)
        self.assertEqual(rule.storage_transitions[1].days, 20)
        self.assertEqual(rule.storage_transitions[1].storage_class, aliyun_oss_x.BUCKET_STORAGE_CLASS_IA)
        self.assertEqual(rule.storage_transitions[2].created_before_date, date1)
        self.assertEqual(rule.storage_transitions[2].storage_class, aliyun_oss_x.BUCKET_STORAGE_CLASS_ARCHIVE)
...
<Error>
  ...
  <HostId>sbowspxjhmccpmesjqcwagfw.oss-cn-hangzhou.aliyuncs.com</HostId>
        ...
        self.assertRaises(aliyun_oss_x.exceptions.AccessDenied, bucket.get_bucket_stat)
        self.assertRequest(req_info, request_text)
...
x-oss-request-id: 566B6BD7D9816D686F72A86A"""
        ...
        unittests.common.bucket().put_bucket_cors(aliyun_oss_x.models.BucketCors([rule]))
...
x-oss-request-id: 566B6BE244ABFA2608E5A8AD"""
        ...
        unittests.common.bucket().put_bucket_referer(
            BucketReferer(True, ["http://hello.com", "mibrowser:home", "阿里巴巴"])
        )
...
<CreateLiveChannelResult>
  ...
  <PlayUrls>
    <Url>http://ming-oss-share.oss-cn-hangzhou.aliyuncs.com/lc/test.m3u8</Url>
        ...
        channel_target = aliyun_oss_x.models.LiveChannelInfoTarget(playlist_name="test.m3u8")
        channel_info = aliyun_oss_x.models.LiveChannelInfo(target=channel_target)
    ...
    def test_list_live_channel(self, do_request):
        from aliyun_oss_x.utils import iso8601_to_unixtime
  ...
  <LiveChannel>
    ...
    <PlayUrls>
      <Url>http://ming-oss-share.oss-cn-hangzhou.aliyuncs.com/test2/playlist.m3u8</Url>
        ...
        self.assertEqual(
            result.channels[0].publish_url, "rtmp://ming-oss-share.oss-cn-hangzhou.aliyuncs.com/live/test1"
        )
        self.assertEqual(
            result.channels[0].play_url, "http://ming-oss-share.oss-cn-hangzhou.aliyuncs.com/test1/playlist.m3u8"
        )
        ...
        self.assertEqual(
            result.channels[1].publish_url, "rtmp://ming-oss-share.oss-cn-hangzhou.aliyuncs.com/live/test2"
        )
        self.assertEqual(
            result.channels[1].play_url, "http://ming-oss-share.oss-cn-hangzhou.aliyuncs.com/test2/playlist.m3u8"
        )

    @patch("aliyun_oss_x.Session.do_request")
    def test_get_live_channel_stat(self, do_request):
        from aliyun_oss_x.utils import iso8601_to_unixtime
        from aliyun_oss_x.models import LiveChannelAudioStat, LiveChannelVideoStat
...
"""
    ...
    def test_get_live_channel_history(self, do_request):
        from aliyun_oss_x.models import LiveRecord
        from aliyun_oss_x.utils import iso8601_to_unixtime
...
"""
        ...
        lr = LiveRecord(
            iso8601_to_unixtime("2016-08-06T05:59:28.000Z"),
        ...
        lr = LiveRecord(
            iso8601_to_unixtime("2016-08-06T06:16:20.000Z"),
