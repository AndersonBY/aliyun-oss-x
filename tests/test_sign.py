...
class TestSign(OssTestCase):
    def test_sign_v1_x_oss_date(self):
        ...
        headers["x-oss-date"] = aliyun_oss_x.utils.http_date(int(time.time()) + 24 * 60 * 60)
        try:
        ...
        headers["x-oss-date"] = aliyun_oss_x.utils.http_date(int(time.time()) + 60)
    ...
    def test_sign_v2_x_oss_date(self):
        auth = aliyun_oss_x.AuthV2(OSS_ID, OSS_SECRET)
    ...
    def test_sign_v4_x_oss_date(self):
        ...
        dest_bucket = aliyun_oss_x.Bucket(auth, OSS_ENDPOINT, dest_bucket_name, region=OSS_REGION)
    ...
    def test_sign_key_is_key_strictly(self):
        ...
        bucket2 = aliyun_oss_x.Bucket(auth, OSS_ENDPOINT, bucket_name, is_verify_object_strict=False)
    ...
    def test_sign_v2_additional_headers(self):
        ...
        try:
            ...
            # additional_headers
            url = dest_bucket.sign_url(
                "PUT", additional_key, 604800, slash_safe=True, headers=headers, additional_headers=additional_headers
            )
            print(url)
            dest_bucket.put_object_with_url(url, "Hello OSS", headers=headers)
            get_url = dest_bucket.sign_url(
                "GET", additional_key, 604800, slash_safe=True, headers=headers, additional_headers=additional_headers
            )
    ...
    def test_sign_v4_no_region(self):
        ...
        dest_bucket = aliyun_oss_x.Bucket(auth, OSS_ENDPOINT, dest_bucket_name)
        region_bucket = aliyun_oss_x.Bucket(auth, OSS_ENDPOINT, dest_bucket_name, region=OSS_REGION)
    ...
    def test_sign_v4_additional_headers(self):
        ...
        try:
            ...
            self.assertTrue(
                url[url.index("=") :].__contains__("x-oss-additional-headers=content-length%3Bcustom-header&")
            )
            dest_bucket.put_object_with_url(url, "Hello OSS", headers=headers)
            get_url = dest_bucket.sign_url(
                "GET", additional_key, 604800, slash_safe=True, headers=headers, additional_headers=additional_headers
            ...
            self.assertTrue(
                get_url[get_url.index("=") :].__contains__("x-oss-additional-headers=content-length%3Bcustom-header")
            )
