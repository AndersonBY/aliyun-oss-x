...
class TestLiveChannel(OssTestCase):
    ...
    def _delete_channels(self):
        prefix = ""
        max_keys = 1000
        for info in aliyun_oss_x.LiveChannelIterator(self.bucket, prefix, max_keys=max_keys):
            self.bucket.delete_live_channel(info.name)

    def _assert_list_result(
        self, result, marker="", prefix="", next_marker="", max_keys=0, is_truncated=False, return_count=0
    ):
    ...
    def test_create_live_channel(self):
        ...
        channel_target = aliyun_oss_x.models.LiveChannelInfoTarget(playlist_name=playlist_name)
        ...
        self.assertEqual(
            create_result.play_url, self._get_play_url(self.bucket.bucket_name, channel_name, playlist_name)
        )
        self.assertEqual(create_result.publish_url, self._get_publish_url(self.bucket.bucket_name, channel_name))
    ...
    def test_list_live_channel(self):
        ...
        self._assert_list_result(
            list_result, prefix="", marker="", next_marker="", max_keys=100, is_truncated=False, return_count=0
        )
        ...
        self._assert_list_result(
            list_result, prefix=prefix2, marker="", next_marker="", max_keys=100, is_truncated=False, return_count=0
        )

        marker = prefix1 + self._get_fixed_number(10, 100)
        list_result = bucket.list_live_channel(prefix=prefix1, marker=marker)
        self._assert_list_result(
            list_result,
            prefix=prefix1,
            marker=marker,
        ...
        self._assert_list_result(
            list_result, prefix="", marker="", next_marker="", max_keys=max_keys, is_truncated=False, return_count=200
        )
    ...
    def test_put_live_channel_status(self):
        ...
        channel_target = aliyun_oss_x.models.LiveChannelInfoTarget()
        channel_info = aliyun_oss_x.models.LiveChannelInfo(target=channel_target)
        self.bucket.create_live_channel(channel_name, channel_info)
    ...
    def test_post_vod_playlist(self):
        ...
        self.assertRaises(
            aliyun_oss_x.exceptions.InvalidArgument,
    ...
    def test_anonymous_auth(self):
        ...
        bucket = aliyun_oss_x.Bucket(aliyun_oss_x.AnonymousAuth(), OSS_ENDPOINT, self.bucket.bucket_name)
        signed_url = bucket.sign_rtmp_url(channel_name, playlist_name, 0)
        self.assertEqual(
            signed_url, self._get_publish_url(self.bucket.bucket_name, channel_name) + "?playlistName=" + playlist_name
        )

        self.assertRaises(aliyun_oss_x.exceptions.AccessDenied, bucket.delete_live_channel, "test-live-chan")
