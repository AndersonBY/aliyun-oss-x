...
{1}""".format(len(content), aliyun_oss_x.to_string(content))
...
""".format(key, len(content), wrap_alg, cek_alg, encrypted_key, encrypted_iv, len(encrypted_content))
    ...
    io = BytesIO()
    io.write(aliyun_oss_x.to_bytes(request_text))
...
""".format(
    ...
    io = BytesIO()
    io.write(aliyun_oss_x.to_bytes(response_text))
    io.write(encrypted_content)
...
{2}""".format(position, len(content), aliyun_oss_x.to_string(content))
...
"""
        ...
        self.assertRaises(
            aliyun_oss_x.exceptions.NoSuchBucket, unittests.common.bucket().object_exists, "sbowspxjhmccpmesjqcwagfw"
        )
...
Server: AliyunOSS"""
    ...
    def test_get_with_progress(self, do_request):
        ...
        self.assertEqual(aliyun_oss_x.to_bytes(content_read), content)
    ...
    def test_get_to_file(self, do_request):
        ...
        with open(filename, "rb") as f:
            self.assertEqual(content, aliyun_oss_x.to_bytes(f.read()))
    ...
    def test_get_to_file_with_progress(self, do_request):
        ...
        self.previous = -1
        unittests.common.bucket().get_object_to_file(
            "sjbhlsgsbecvlpbf", filename, progress_callback=self.progress_callback
        )
        ...
        with open(filename, "rb") as f:
            self.assertEqual(aliyun_oss_x.to_bytes(content), f.read())
...
ETag: "D80CF0E5BE2436514894D64B2BCFB2AE"'''
    ...
    def test_append_with_progress(self, do_request):
        ...
        result = unittests.common.bucket().append_object(
            "sjbhlsgsbecvlpbf", 0, content, progress_callback=self.progress_callback
        )
...
x-oss-request-id: 566B6C0D8CDE4E975D730BEF"""
    ...
    def test_batch_delete_empty(self):
        self.assertRaises(aliyun_oss_x.exceptions.ClientError, unittests.common.bucket().batch_delete_objects, [])
...
<Deleted>
    <Key>world</Key>
        ...
        self.assertRequest(req_info, request_text)
        self.assertEqual(result.deleted_keys, list(aliyun_oss_x.to_string(key) for key in key_list))
...
<CopyObjectResult>
  ...
  <LastModified>2015-12-12T00:37:53.000Z</LastModified>
    ...
    def test_put_acl(self, do_request):
        ...
        for acl, expected in [
            (aliyun_oss_x.OBJECT_ACL_PRIVATE, "private"),
            (aliyun_oss_x.OBJECT_ACL_PUBLIC_READ, "public-read"),
            (aliyun_oss_x.OBJECT_ACL_PUBLIC_READ_WRITE, "public-read-write"),
            (aliyun_oss_x.OBJECT_ACL_DEFAULT, "default"),
        ]:
    ...
    def test_get_acl(self, do_request):
        ...
        for acl, expected in [
            ...
            do_request.auto_spec = True
            do_request.side_effect = partial(
                unittests.common.do4body, body=template.format(acl), content_type="application/xml"
            )
...
x-oss-server-time: 39"""
    ...
    def test_crypto_get(self, do_request):
        ...
        provider = aliyun_oss_x.RsaProvider(key_pair=key_pair)
    ...
    def test_crypto_get_with_inconsistent_crypto_meta(self, do_request):
        ...
        request_text, response_text = make_get_encrypted_object(
            key, content, content_crypto_material, invalid_cek_alg="invald_cek_alg"
        )
        req_info = unittests.common.mock_response(do_request, response_text)

        self.assertRaises(aliyun_oss_x.exceptions.InconsistentError, unittests.common.bucket(provider).get_object, key)
    ...
    def test_crypto_get_compact(self, do_request):
        ...
        for compact_sdk in ["cpp", "go"]:
            ...
            key = random_string(10)
            provider = aliyun_oss_x.RsaProvider(key_pair=key_pair_compact)
    ...
    def test_crypto_get_compact_deprecated_rsa(self, do_request):
        ...
        with open("./rsa-test.private_key.pem", "wb") as f:
            f.write(aliyun_oss_x.to_bytes(private_key_compact))

        with open("./rsa-test.public_key.pem", "wb") as f:
            f.write(aliyun_oss_x.to_bytes(public_key_compact))
        ...
        key = random_string(10)
        provider = aliyun_oss_x.LocalRsaProvider(dir="./", key="rsa-test")
    ...
    def test_crypto_get_compact_deprecated_kms(self, do_request):
        ...
        key = random_string(10)
        provider = aliyun_oss_x.AliKMSProvider(OSS_ID, OSS_SECRET, OSS_REGION, "cmk-id")
        ...
        req_info = unittests.common.mock_response(do_request, response_text)
        with patch("aliyun_oss_x.AliKMSProvider.decrypt_encrypted_key", return_value=plain_key):
            with patch("aliyun_oss_x.AliKMSProvider.decrypt_encrypted_iv", return_value=plain_start):
                result = unittests.common.bucket(provider).get_object(key)
    ...
    def test_crypto_put(self, do_request):
        ...
        with patch.object(aliyun_oss_x.utils, "random_key", return_value=plain_key):
            with patch.object(aliyun_oss_x.utils, "random_iv", return_value=plain_iv):
                content_crypto_material = provider.create_content_material()
        ...
        with patch.object(aliyun_oss_x.utils, "random_key", return_value=plain_key):
            with patch.object(aliyun_oss_x.utils, "random_iv", return_value=plain_iv):
                result = unittests.common.bucket(provider).put_object(
                    key,
                    content,
                    headers={
                        "content-md5": aliyun_oss_x.utils.md5_string(content),
    ...
    def test_oss_utils_negative(self):
        try:
            aliyun_oss_x.utils.makedir_p("/")
            self.assertTrue(False)
        except:
            pass

        try:
            aliyun_oss_x.utils.silently_remove("/")
            self.assertTrue(False)
        except:
            pass

        try:
            aliyun_oss_x.utils.force_rename("/", "/")
            self.assertTrue(False)
        except:
            pass

        aliyun_oss_x.utils.makedir_p("xyz")
        aliyun_oss_x.utils.makedir_p("zyz")
        try:
            aliyun_oss_x.utils.force_rename("xyz", "zyx")
            self.assertTrue(False)
        except:
            pass
