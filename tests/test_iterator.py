...
class TestIterator(OssTestCase):
    def test_bucket_iterator(self):
        ...
        self.assertTrue(self.OSS_BUCKET in (b.name for b in aliyun_oss_x.BucketIterator(service, max_keys=2)))
    ...
    def test_object_iterator(self):
        ...
        objects_got = []
        dirs_got = []
        for info in aliyun_oss_x.ObjectIterator(self.bucket, prefix, delimiter="/", max_keys=4):
            if info.is_prefix():
    ...
    def test_upload_iterator(self):
        ...
        uploads_got = []
        dirs_got = []
        for u in aliyun_oss_x.MultipartUploadIterator(self.bucket, prefix=prefix, delimiter="/", max_uploads=2):
            if u.is_prefix():
    ...
    def test_upload_iterator_chinese(self):
        ...
        uploads_got = []
        for prefix in prefix_list:
            listed = list(aliyun_oss_x.MultipartUploadIterator(self.bucket, prefix=prefix, max_uploads=1))
    ...
    def test_object_upload_iterator(self):
        ...
        uploads_got = []
        for u in aliyun_oss_x.ObjectUploadIterator(self.bucket, target_object, max_uploads=5):
            uploads_got.append(u.upload_id)
        ...
        uploads_got = []
        for u in aliyun_oss_x.ObjectUploadIterator(self.bucket, target_object, max_uploads=3):
            uploads_got.append(u.upload_id)
    ...
    def test_part_iterator(self):
        for key in [random_string(16), "中文+_)(*&^%$#@!前缀", "中文+_)(*&^%$#@!前缀"]:
            ...
            for part_number in [1, 3, 6, 7, 9, 10]:
                ...
                etag = hashlib.md5(aliyun_oss_x.to_bytes(content)).hexdigest().upper()
                part_list.append(aliyun_oss_x.models.PartInfo(part_number, etag, len(content)))

                self.bucket.upload_part(key, upload_id, part_number, content)

            # 验证
            parts_got = []
            for part_info in aliyun_oss_x.PartIterator(self.bucket, key, upload_id):
    ...
    def test_live_channel_iterator(self):
        ...
        # 验证
        live_channel_got = []
        for info in aliyun_oss_x.LiveChannelIterator(self.bucket, prefix, max_keys=4):
