...
def random_bytes(n):
    return aliyun_oss_x.to_bytes(random_string(n))
...
def clean_and_delete_bucket(bucket):
    ...
    try:
        ...
        if result.versioning_status in [aliyun_oss_x.BUCKET_VERSIONING_ENABLE, aliyun_oss_x.BUCKET_VERSIONING_SUSPEND]:
            next_key_marker = None
            next_versionid_marker = None
            is_truncated = True
            while is_truncated is True:
                objects = bucket.list_object_versions(
                    key_marker=next_key_marker, versionid_marker=next_versionid_marker
                )
        ...
        logging.error(f"clean bucket failed: {e}")

    # list all upload_parts to delete
    up_iter = aliyun_oss_x.MultipartUploadIterator(bucket)
    for up in up_iter:
        bucket.abort_multipart_upload(up.key, up.upload_id)

    # list all objects to delete
    obj_iter = aliyun_oss_x.ObjectIterator(bucket)
    for obj in obj_iter:
        bucket.delete_object(obj.key)

    # list all live channels to delete
    for ch_iter in aliyun_oss_x.LiveChannelIterator(bucket):
...
def clean_and_delete_bucket_by_prefix(bucket_prefix):
    ...
    for b in buckets:
        bucket = aliyun_oss_x.Bucket(
            aliyun_oss_x.Auth(OSS_ID, OSS_SECRET), b.extranet_endpoint, b.name, region=OSS_REGION
        )
...
class OssTestCase(unittest.TestCase):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.bucket = aliyun_oss_x.Bucket(
            aliyun_oss_x.make_auth(OSS_ID, OSS_SECRET),
            OSS_ENDPOINT,
            self.OSS_BUCKET,
            region=OSS_REGION,
        )
        self.prefix = random_string(12)
        self.default_connect_timeout = aliyun_oss_x.defaults.connect_timeout
        self.default_multipart_num_threads = aliyun_oss_x.defaults.multipart_threshold
    ...
    def setUp(self):
        aliyun_oss_x.defaults.connect_timeout = self.default_connect_timeout
        aliyun_oss_x.defaults.multipart_threshold = self.default_multipart_num_threads
        aliyun_oss_x.defaults.multipart_num_threads = random.randint(1, 5)

        aliyun_oss_x.defaults.multiget_threshold = self.default_multiget_threshold
        aliyun_oss_x.defaults.multiget_part_size = self.default_multiget_part_size
        aliyun_oss_x.defaults.multiget_num_threads = random.randint(1, 5)
        ...
        try:
            ...
            logging.error(f"create kms-id-object failed: {e}")

        self.rsa_crypto_bucket = aliyun_oss_x.CryptoBucket(
            aliyun_oss_x.make_auth(OSS_ID, OSS_SECRET),
            OSS_ENDPOINT,
            self.OSS_BUCKET,
            crypto_provider=aliyun_oss_x.RsaProvider(key_pair),
            region=OSS_REGION,
        )

        self.kms_crypto_bucket = aliyun_oss_x.CryptoBucket(
            aliyun_oss_x.make_auth(OSS_ID, OSS_SECRET),
            OSS_ENDPOINT,
            self.OSS_BUCKET,
            crypto_provider=aliyun_oss_x.AliKMSProvider(OSS_ID, OSS_SECRET, OSS_REGION, self.KMS_CMK_ID),
            region=OSS_REGION,
        )
    ...
    def tearDown(self):
        for temp_file in self.temp_files:
            aliyun_oss_x.utils.silently_remove(temp_file)
    ...
    def _prepare_temp_file_with_size(self, size):
        ...
        while num_written < size:
            ...
            content = "s" * to_write
            os.write(fd, aliyun_oss_x.to_bytes(content))
