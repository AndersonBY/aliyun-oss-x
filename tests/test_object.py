...
class TestObject(OssTestCase):
    ...
    def test_restore_object_with_config(self):
        ...
        bucket = aliyun_oss_x.Bucket(
            aliyun_oss_x.make_auth(OSS_ID, OSS_SECRET, OSS_AUTH_VERSION), OSS_ENDPOINT, bucket_name, region=OSS_REGION
        )
        bucket.create_bucket()
        ...
        for index in range(0, len(tiers)):
            ...
            bucket.put_object(
                object_name, "123", headers={"x-oss-storage-class": aliyun_oss_x.BUCKET_STORAGE_CLASS_COLD_ARCHIVE}
            )

            meta = bucket.head_object(object_name)
            self.assertEqual(aliyun_oss_x.BUCKET_STORAGE_CLASS_COLD_ARCHIVE, meta.resp.headers["x-oss-storage-class"])

            job_parameters = RestoreJobParameters(tiers[index])
            restore_config = aliyun_oss_x.models.RestoreConfiguration(days=5, job_parameters=job_parameters)
    ...
    def test_restore_object_with_wrong_request(self):
        from aliyun_oss_x.models import RestoreJobParameters, RestoreConfiguration

        endpoint = OSS_ENDPOINT
        bucket_name = self.OSS_BUCKET + "-test-restore-2"
        bucket = aliyun_oss_x.Bucket(
            aliyun_oss_x.make_auth(OSS_ID, OSS_SECRET, OSS_AUTH_VERSION), endpoint, bucket_name, region=OSS_REGION
        )
        bucket.create_bucket()
        ...
        restore_config = aliyun_oss_x.models.RestoreConfiguration(days=5, job_parameters=job_parameters)
        self.assertRaises(
            aliyun_oss_x.exceptions.InvalidArgument, bucket.restore_object, object_name, input=restore_config
        )

    def test_restore_archive_object_with_job_parameters(self):
        from aliyun_oss_x.models import RestoreJobParameters, RestoreConfiguration, RESTORE_TIER_BULK
        ...
        bucket.put_object(
            object_name, "123", headers={"x-oss-storage-class": aliyun_oss_x.BUCKET_STORAGE_CLASS_ARCHIVE}
        )
        meta = bucket.head_object(object_name)
        self.assertEqual(aliyun_oss_x.BUCKET_STORAGE_CLASS_ARCHIVE, meta.resp.headers["x-oss-storage-class"])

        job_parameters = RestoreJobParameters(RESTORE_TIER_BULK)
        restore_config = aliyun_oss_x.models.RestoreConfiguration(days=5, job_parameters=job_parameters)
        self.assertRaises(
            aliyun_oss_x.exceptions.MalformedXml, bucket.restore_object, object_name, input=restore_config
        )
    ...
    def test_last_modified_time(self):
        ...
        self.assertEqual(aliyun_oss_x.utils.http_to_unixtime(time_string), res.last_modified)
        self.assertEqual(aliyun_oss_x.utils.to_unixtime(time_string, "%a, %d %b %Y %H:%M:%S GMT"), res.last_modified)
    ...
    def test_request_error(self):
        ...
        bucket = aliyun_oss_x.Bucket(aliyun_oss_x.Auth(OSS_ID, OSS_SECRET), bad_endpoint, self.OSS_BUCKET)

        try:
            bucket.get_bucket_acl()
        except RequestError as e:
            self.assertEqual(e.status, aliyun_oss_x.exceptions.OSS_REQUEST_ERROR_STATUS)
    ...
    def test_timeout(self):
        bucket = aliyun_oss_x.Bucket(
            aliyun_oss_x.Auth(OSS_ID, OSS_SECRET), OSS_ENDPOINT, self.OSS_BUCKET, connect_timeout=0.001
        )
        self.assertRaises(RequestError, bucket.get_bucket_acl)

    def test_default_timeout(self):
        aliyun_oss_x.defaults.connect_timeout = 0.001
        bucket = aliyun_oss_x.Bucket(aliyun_oss_x.Auth(OSS_ID, OSS_SECRET), OSS_ENDPOINT, self.OSS_BUCKET)
        self.assertRaises(RequestError, bucket.get_bucket_acl)
        aliyun_oss_x.defaults.connect_timeout = 60
    ...
    def test_get_object_iterator(self):
        ...
        for chunk in result:
            content_got += aliyun_oss_x.to_bytes(chunk)
    ...
    def test_anonymous(self):
        ...
        b = aliyun_oss_x.Bucket(aliyun_oss_x.AnonymousAuth(), OSS_ENDPOINT, self.OSS_BUCKET)
        ...
        self.assertRaises(aliyun_oss_x.exceptions.AccessDenied, b.put_object, key, content)
        self.assertRaises(aliyun_oss_x.exceptions.AccessDenied, b.get_object, key)
    ...
    def test_batch_delete_objects_empty(self):
        try:
            ...
            self.assertEqual(e.status, aliyun_oss_x.exceptions.OSS_CLIENT_ERROR_STATUS)
            self.assertEqual(e.request_id, "")
            self.assertEqual(e.code, "")
            self.assertEqual(e.message, "")
    ...
    def test_private_download_url(self):
        for key in [self.random_key(), self.random_key("中文文件名")]:
    ...
    def test_private_download_url_with_extra_query(self):
        ...
        e = aliyun_oss_x.exceptions.make_exception(aliyun_oss_x.http.Response(resp))
    ...
    def test_put_object_with_sign_url(self):
        ...
        self.assertRaises(
            aliyun_oss_x.exceptions.SignatureDoesNotMatch, self.bucket.put_object_with_url, url, data, headers=headers
        )

        url = self.bucket.sign_url("PUT", key, 3600, headers=headers)
        self.assertRaises(aliyun_oss_x.exceptions.SignatureDoesNotMatch, self.bucket.put_object_with_url, url, data)
    ...
    def test_get_object_with_sign_url_slash_safe(self):
        ...
        slash_safe_key = (
            urlquote("url下载测试斜杠保护+") + "/" + urlquote("二层目录+") + "/" + urlquote("测+试斜杠保护.object")
        )
        content = random_bytes(100)
    ...
    def test_put_object_with_sign_url_slash_safe(self):
        ...
        slash_safe_key = (
            urlquote("url上传测试斜杠保护+") + "/" + urlquote("二层目录+") + "/" + urlquote("测+试斜杠保护.object")
        )
        content = random_bytes(1024)
    ...
    def test_modified_since(self):
        ...
        self.bucket.put_object(key, content)
        self.assertRaises(
            aliyun_oss_x.exceptions.NotModified,
            self.bucket.get_object,
            key,
            headers={
                "if-modified-since": aliyun_oss_x.utils.http_date(int(time.time()) + 60),
            },
            byte_range=(0, 7),
        )
    ...
    def test_update_object_meta(self):
        ...
        self.bucket.update_object_meta(key, {"Content-Type": "whatever", "x-oss-meta-category": "novel"})
    ...
    def test_object_acl(self):
        ...
        self.bucket.put_object(key, content)
        self.assertEqual(self.bucket.get_object_acl(key).acl, aliyun_oss_x.OBJECT_ACL_DEFAULT)

        for permission in (
            aliyun_oss_x.OBJECT_ACL_PRIVATE,
            aliyun_oss_x.OBJECT_ACL_PUBLIC_READ,
            aliyun_oss_x.OBJECT_ACL_PUBLIC_READ_WRITE,
            aliyun_oss_x.OBJECT_ACL_DEFAULT,
        ):
    ...
    def test_user_meta(self):
        ...
        self.bucket.put_object(key, "hello", headers={"x-oss-meta-key1": "value1", "X-Oss-Meta-Key2": "value2"})
    ...
    def test_invalid_object_name(self):
        ...
        self.assertRaises(aliyun_oss_x.exceptions.InvalidObjectName, self.bucket.put_object, key, content)
    ...
    def test_disable_crc(self):
        ...
        bucket = aliyun_oss_x.Bucket(
            aliyun_oss_x.Auth(OSS_ID, OSS_SECRET), OSS_ENDPOINT, self.OSS_BUCKET, enable_crc=False
        )
        ...
        parts.append(aliyun_oss_x.models.PartInfo(1, result.etag))
        result = bucket.upload_part(key, upload_id, 2, content)
        parts.append(aliyun_oss_x.models.PartInfo(2, result.etag))
    ...
    def test_put_symlink(self):
        ...
        self.bucket.put_symlink(key, symlink, headers={"x-oss-meta-key1": "value1", "X-Oss-Meta-Key2": "value2"})
        head_result = self.bucket.head_object(symlink)
        self.assertEqual(head_result.content_length, len(content))
        self.assertEqual(head_result.etag, "5D41402ABC4B2A76B9719D911017C592")
    ...
    def test_append_object_with_tagging_wrong_num(self):
        ...
        try:
            result = self.bucket.append_object(
                key, len(content1) + len(content2), content2, init_crc=result.crc, headers=headers
            )
        except aliyun_oss_x.exceptions.OssError:
