...
from aliyun_oss_x.models import OSS_TRAFFIC_LIMIT, PartInfo
...
class TestTrafficLimit(OssTestCase):
    ...
    def test_resumable_upload(self):
        ...
        result = aliyun_oss_x.resumable_upload(
            self.bucket, key, pathname, multipart_threshold=(OBJECT_SIZE_1MB * 2), headers=headers, num_threads=1
        )
        self.assertEqual(result.status, 200)
        end_time_sec = int(time.time())
        ...
        start_time_sec = int(time.time())
        result = aliyun_oss_x.resumable_upload(
            self.bucket, key, pathname, multipart_threshold=(OBJECT_SIZE_1MB - 1024), headers=headers, num_threads=1
        )
        self.assertEqual(result.status, 200)
        end_time_sec = int(time.time())
    ...
    def test_resumable_download(self):
        ...
        aliyun_oss_x.resumable_download(
            self.bucket, key, file_name, multiget_threshold=(OBJECT_SIZE_1MB * 2), num_threads=1, headers=headers
        )
        ...
        start_time_sec = int(time.time())
        aliyun_oss_x.resumable_download(
            self.bucket, key, file_name, multiget_threshold=(OBJECT_SIZE_1MB - 1024), num_threads=1, headers=headers
        )
        self.assertFileContent(file_name, content)
        end_time_sec = int(time.time())
