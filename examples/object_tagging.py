...
from aliyun_oss_x import SizedFileAdapter, determine_part_size
from aliyun_oss_x.headers import OSS_OBJECT_TAGGING
...
k4 = "k4+-="
v4 = "+-=._:/"
tagging += "&" + aliyun_oss_x.urlquote(k4) + "=" + aliyun_oss_x.urlquote(v4)
...
with open(filename, "rb") as fileobj:
    ...
    while offset < total_size:
        ...
        result = bucket.upload_part(object_name, upload_id, part_number, SizedFileAdapter(fileobj, num_to_upload))
...
aliyun_oss_x.resumable_upload(bucket, object_name, local_file, headers=headers)
...
while offset < total_size:
    ...
    result = bucket.upload_part_copy(
        bucket.bucket_name, src_object_name, (offset, end), dest_object_name, upload_id, part_number
    )
...
rule5 = LifecycleRule(
    ...
    "logging2-",
    status=LifecycleRule.ENABLED,
    storage_transitions=[StorageTransition(days=365, storage_class=aliyun_oss_x.BUCKET_STORAGE_CLASS_ARCHIVE)],
    tagging=tagging,
)
...
for rule in lifecycle.rules:
    ...
    if rule.abort_multipart_upload is not None:
        print(
            "id={0}, prefix={1}, tagging={2}, status={3}, days={4}, created_before_date={5}".format(
                rule.id,
                rule.prefix,
                rule.tagging,
                rule.status,
    ...
    if len(rule.storage_transitions) > 0:
        ...
        print(
            "id={0}, prefix={1}, tagging={2}, status={3},, StorageTransition={4}".format(
                rule.id, rule.prefix, rule.tagging, rule.status, storage_trans_info
            )
        )
