...
key = "motto.txt"
content = aliyun_oss_x.to_bytes("a" * 1024 * 1024)
...
for chunk in result:
    content_got += chunk
assert content_got == aliyun_oss_x.to_bytes("a" * 1024)
...
# 验证一下
with open(filename, "rb") as fileobj:
    assert fileobj.read() == aliyun_oss_x.to_bytes("a" * 1024)
...
aliyun_oss_x.resumable_download(
    bucket, key, filename, multiget_threshold=200 * 1024, part_size=100 * 1024, num_threads=3
)
