...
for i, object_info in enumerate(aliyun_oss_x.ObjectIterator(bucket)):
...
# 获取不存在的文件会抛出aliyun_oss_x.exceptions.NoSuchKey异常
try:
    bucket.get_object("云上座右铭.txt")
except aliyun_oss_x.exceptions.NoSuchKey as e:
    print("已经被删除了：request_id={0}".format(e.request_id))
else:
    assert False
