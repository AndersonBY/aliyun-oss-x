...
create_result = bucket.create_live_channel(
    channel_name,
    aliyun_oss_x.models.LiveChannelInfo(
        status="enabled",
        description="测试使用的直播频道",
        target=aliyun_oss_x.models.LiveChannelInfoTarget(playlist_name=playlist_name, frag_count=3, frag_duration=5),
    ),
)
...
for info in aliyun_oss_x.LiveChannelIterator(bucket, prefix, max_keys=max_keys):
    print(info.name)
...
bucket.post_vod_playlist(channel_name, playlist_name, start_time=start_time, end_time=end_time)
