...
def calculate_file_crc64(file_name, block_size=64 * 1024, init_crc=0):
    ...
    """
    with open(file_name, "rb") as f:
        crc64 = aliyun_oss_x.utils.Crc64(init_crc)
        while True:
            data = f.read(block_size)
            if not data:
                break
...
CRC64校验
"""
# CRC校验默认打开。关闭CRC校验方法如下，aliyun_oss_x.Bucket(auth, endpoint, bucket_name, enable_crc=False)
...
aliyun_oss_x.resumable_upload(
    bucket, key, pathname, multipart_threshold=200 * 1024, part_size=100 * 1024, num_threads=3
)
...
aliyun_oss_x.resumable_download(
    bucket, key, local_file, multiget_threshold=200 * 1024, part_size=100 * 1024, num_threads=3
)
